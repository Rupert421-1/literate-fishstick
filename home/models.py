...
def post_order_on_telegram(sender, instance, created, **kwargs):
    if created:
        asyncio.run(post_event_on_telegram(instance.from_where, instance.to_where, instance.weight, instance.when, instance.truck_type))

async def post_event_on_telegram(from_where, to_where, weight, when, truck_type):
    event = {
